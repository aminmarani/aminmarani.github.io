...
class Post_Author extends Block_Abstract {
				...
				if ( ! empty( $author ) ) {
					switch ( $link_to ) {
						...
						case 'custom':
							...
							$content = '<' . $html_tag . '>
                                <a href="' . $custom_url . '">' . $author . '</a>
                            </' . $html_tag . '>';
							break;
						default:
							$content = '<' . $html_tag . '>' . $author . '</' . $html_tag . '>';
							break;
					}
	...
	public function render_frontend() {
		...
		return '<div class="' . $element_id . $display_classes . ' guten-post-author guten-element">' . $this->render_content( $post_id ) . '</div>';
	}
}
