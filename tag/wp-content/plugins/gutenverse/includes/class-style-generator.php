...
class Style_Generator {
	...
	public function flatten_blocks( $blocks ) {
		if ( is_gutenverse_compatible() ) {
			// use Gutenberg or WP 5.9 version.
	...
	public function get_template_part_content( $attributes ) {
		...
		if (
			...
			if ( $template_part_post ) {
				...
				$template_part_file_path = get_theme_file_path( '/parts/' . $attributes['slug'] . '.html' );
				if ( 0 === validate_file( $attributes['slug'] ) && file_exists( $template_part_file_path ) ) {
					$content = file_get_contents( $template_part_file_path );
					$content = is_string( $content ) && '' !== $content
							? $this->inject_theme_attribute_in_block_template_content( $content )
							: '';
				}
			}
		}
