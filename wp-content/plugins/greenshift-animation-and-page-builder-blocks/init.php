...
function gspb_greenShift_register_scripts_blocks()
{
	...
	wp_register_style('swiper', GREENSHIFT_DIR_URL . 'libs/swiper/swiper-bundle.min.css', array(), '8.0');
...
function gspb_greenShift_block_script_assets($html, $block)
{
	...
	if(!is_admin()){
		...
		if ($block['blockName'] === 'greenshift-blocks/button' && !empty( $block['attrs']['slidingPanel'])) {
			wp_enqueue_script('gsslidingpanel');
		}

		// looking for sliding panel
		if ($block['blockName'] === 'greenshift-blocks/container' && !empty( $block['attrs']['flipbox'])) {
...
function gspb_greenShift_editor_assets()
{
	...
	wp_localize_script(
		...
		array(
			...
			'pluginURL' => GREENSHIFT_DIR_URL,
			'isAnimationPRO' => defined('GREENSHIFTGSAP_DIR_URL'),
...
function gspb_save_inline_css()
{
	...
	if(!$gspb_css_content){
		$gspb_css_content_old = get_post_meta($post_id, 'gspb_post_css', true);
		$gspb_css_content = $gspb_css_content_old;
	}
	if($gspb_css_content){
		$gspb_saved_css_content = gspb_get_final_css($gspb_css_content);
		$final_css = $gspb_saved_css_content;
...
function gspb_update_global_settings($request)
{
	...
	try {
		...
		if (!$wp_filesystem->put_contents($dir . $gspb_css_filename, $gs_global_css)) {
			throw new Exception(__('CSS not saved due the permission!!!', 'fragmental-theme'));
		}
...
const TEMPLATE_SERVER_URL = 'https://library.wpsoul.net/';
...
if(!function_exists('gspb_get_all_layouts')){
	function gspb_get_all_layouts()
	{
		...
		$apiUrl   = TEMPLATE_SERVER_URL . '/wp-json/wp/v2/posts/?_embed&categories=' . $category . '&per_page=200';
